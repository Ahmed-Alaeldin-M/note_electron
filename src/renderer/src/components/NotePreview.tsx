import { ComponentProps } from "react"
import { NoteInfo } from "@shared/models"
import { cn, formatDate } from "@renderer/utils"
export type NotePreviewProps = NoteInfo & {
    isActive?:boolean
}& ComponentProps<'div'>
export const NotePreview = ({className,title,isActive=false,content,lastEdited,...props}:NotePreviewProps)=>{
    const date = formatDate(lastEdited)
    return(
        <div className={cn('cursor-pointer px-2.5 py-3 rounded-md transition-colors duration-75',
        {
            'bg-zinc-400/75':isActive,
            'hover:bg-zinc-500/75':!isActive
        }
        ,className)} {...props}>
            <h3 className="mb-1 font-bold truncate">
            {title}
            </h3>
            <span className="inline-block w-full mb-2 font-light text-left text-xs text-zinc-300">
            {date}
            </span>
         
        </div>
    )
}